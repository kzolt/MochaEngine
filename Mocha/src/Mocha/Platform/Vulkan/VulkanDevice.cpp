...
namespace Mocha {
	...
	VulkanPhysicalDevice::VulkanPhysicalDevice()
	{
		...
		{
			MC_CORE_TRACE("Could not find discrete GPU.");
			selectedPhysicalDevice = physicalDevices.back();
		}

		MC_CORE_ASSERT(selectedPhysicalDevice, "Could not find any physical devices!");
		...
		MC_CORE_ASSERT(queueFamilyCount > 0);
		...
		if (extCount > 0)
		{
			...
			{
				MC_CORE_TRACE("Selected physical device has {0} extensions", extensions.size());
				for (const auto& ext : extensions)
				{
					m_SupportedExtensions.emplace(ext.extensionName);
					MC_CORE_TRACE("\t{0}", ext.extensionName);
				}
			}
		}
	...
	uint32_t VulkanPhysicalDevice::GetMemoryTypeIndex(uint32_t typeBits, VkMemoryPropertyFlags properties) const
	{
		...
		MC_CORE_ASSERT(false, "Could not find a suitable memory type!");
		return UINT32_MAX;
	}
	...
	VulkanPhysicalDevice::QueueFamilyIndices VulkanPhysicalDevice::GetQueueFamilyIndices(int flags)
	{
		...
		// Try to find a queue family index that supports transfer but not graphics and computer
		...
		: m_PhysicalDevice(physicalDevice), m_EnabledFeatures(enabledFeatures)
	{
		...
		MC_CORE_ASSERT(m_PhysicalDevice->IsExtensionSupported(VK_KHR_SWAPCHAIN_EXTENSION_NAME));
	...
	void VulkanDevice::FlushCommandBuffer(VkCommandBuffer commandBuffer)
	{
		...
		MC_CORE_ASSERT(commandBuffer != VK_NULL_HANDLE);
